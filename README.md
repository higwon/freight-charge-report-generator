...
- 영문 헤더와 한국어 헤더를 모두 인식합니다.
- 원본 데이터를 첫 번째 `Source` 시트에 그대로 유지합니다.
- `AR/AP 월별 보고서`, `요약 보고서` 중 하나를 선택해 생성할 수 있습니다.
- `AR/AP 월별 보고서`는 `Overview`와 물류코드별 상세 시트를 생성합니다.
- 월별보고서 Overview에는 전체 KPI, 코드별 요약, 월별 요약, 매출·매입 상위 Port, 차트가 포함됩니다.
- 각 코드별 시트에는 코드 전체 월별 요약, 항구별 누적 요약, 매출·매입 Port Top 5가 포함됩니다.
- 수출 코드 `AE`, `OE`, `OA`는 도착항 기준으로 집계하고, 펼치면 거래처 상세를 표시합니다.
- 수입 코드 `AI`, `OI`는 출발항 기준으로 집계하고, 펼치면 거래처 상세를 표시합니다.
- 그 외 물류코드는 `항구 없음` 그룹 기준으로 집계하고, 펼치면 거래처 상세를 표시합니다.
...
상세
기본정보 | 누적 합계 | 월별 그룹
항구 | 거래처코드 | 거래처명 | 누적 매출계 | 누적 매입계 | 누적 마진 | 누적 마진율 | 월별 AR계/AP계/마진/마진율...
```

상세 표는 항구 요약행을 먼저 표시하고, 항구 행을 펼치면 해당 항구의 거래처별 금액을 확인할 수 있습니다.
항구 기준이 없는 물류코드는 `항구 없음` 그룹으로 묶어 동일한 방식으로 표시합니다.
...
- `Overview`: Func Code별, 월별 합계와 상위 Port 및 차트
